...
if is_train:
      # 데이터 불러오기
      df = pd.read_csv(workspace + "data/" + dataVersion + ".csv", encoding="UTF-8")
      ...
      # 임베딩 모델 저장 및 로드
      embedding_model.wv.save_word2vec_format(workspace + "data/tokens/tokens_"+dataVersionPath)
      loaded_model = KeyedVectors.load_word2vec_format(workspace + "data/tokens/tokens_"+dataVersionPath)

      # 데이터셋 분할 및 저장
      rng = RandomState()
      tr = df_drop.sample(frac=0.8, random_state=rng)
      val = df_drop.loc[~df_drop.index.isin(tr.index)]
      tr.to_csv(workspace + 'data/train_'+dataVersionPath+'.csv', index=False, encoding='utf-8-sig')
      val.to_csv(workspace + 'data/validation_'+dataVersionPath+'.csv', index=False, encoding='utf-8-sig')
...
test = TabularDataset(
      path = workspace + 'data/validation_'+dataVersionPath+'.csv',
      format = 'csv',
      fields = [('text', TEXT), ('label', LABEL)],
      skip_header = True
)
...
vectors = Vectors(name = workspace + "data/tokens/tokens_"+dataVersionPath)
...
if is_train:
      ...
      for epoch in range(1, 101):
            ...
            if val_acc > best_test_acc:
                  ...
                  torch.save(model.state_dict(), workspace + "result/Best_Validation_"+dataVersionPath)
      ...
      model.load_state_dict(torch.load(workspace + "result/Best_Validation_v2_3"))
